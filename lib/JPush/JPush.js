...
exports.buildClient = function(appKey, masterSecret, retryTimes, isDebug, opts) {
    if (arguments.length == 1 && typeof arguments[0] == 'object') {
        var options = arguments[0];
        return new JPushClient(options.appKey, options.masterSecret, options.retryTimes, options.isDebug, options.opts || {});
    } else {
        return new JPushClient(appKey, masterSecret, retryTimes, isDebug, opts || {});
    }
};

function JPushClient(appKey, masterSecret, retryTimes, isDebug, options) {
...
function sendPush(payload, callback) {
    ...
    }, 'POST', 1, this.retryTimes, this.options.proxy, this.isDebug, callback);
}
...
function getReportReceiveds(msg_ids, callback) {
    ...
    return _request(url, null, header, {
        user : this.appkey,
        pass : this.masterSecret
    }, 'GET', 1, this.retryTimes, this.options.proxy, this.isDebug, callback);
}
...
function deleteTag(theTag, platform, callback) {
    ...
    return _request(url, null, header, {
        user : this.appkey,
        pass : this.masterSecret
    }, 'delete', 1, this.retryTimes, this.options.proxy, this.isDebug, callback);
}
...
function _request(url, body, headers, auth, method, times, maxTryTimes, proxy, isDebug, callback) {
    if (isDebug) {
        ...
        if (err) {
            ...
            if (times < maxTryTimes) {
                return _request(url, body, headers, auth, method, times + 1,
                        maxTryTimes, proxy, isDebug, callback);
            } else {
                return callback(new JError.APIConnectionError(
    ...
    Request[{'post': 'post', 'get': 'get', 'delete': 'del'}[method.toLowerCase()]](options, _callback);
}
